...
#trig functions
#build for econ as well
#(table and graph)

# Window
SCREENSIZE = (600, 600)
TITLE = "PyGraph 0.8.01"
...
y_min = -10
y_max = 10
#make input functions for it before startup
...
def interest(P,r,t,n=None):
    if n==None:
        A=P*(math.e**(r*t))
    else:
        A=P*(1+(r/n))**(n*t)
    return A
def multiply_interest(rate, multiplier):
    return math.log(multiplier)/rate
def exp_growth(amount,rate,time):
    a,r,t=amount,rate,time
    return a*math.e**(r*t)
def exp_decay(amount,rate,time):
    a,r,t=amount,rate,time
    return a*math.e**(-(r*t))
def carbon_dating(num_isotopes=1/10**12,
                  halflife=5700,
                  time=1000):
    num,h,t=num_isotopes,halflife,time
    R=num*math.e**(-t/halflife)
    return R

system01 = [{"equation":lambda x,y,z:(6*x)-y+z,
             "answer"  :-1},
            {"equation":lambda x,y,z:(4*x)-(3*z),
             "answer"  :-19},
            {"equation":lambda x,y,z:(2*y)+(5*z),
             "answer"  :25}]

def multilinear_system(system,solution):
    n = len(solution)
    for s in system:
        if n == 3:
            a = s['equation'](solution[0],solution[1],solution[2])
        if n == 2:
            a = s['equation'](solution[0],solution[1])
        if n == 1:
            a = s['equation'](solution[0])
        a = a
        if a != s['answer']:
            return False
    return True

def augment_matrix(matrix, order, solution):
    len_e = order[1]-1
    for m in matrix:
        if matrix.index(m)+1 % order[1] == 0:
            answer=m
            start=matrix.index(m)-len_e
            equation=matrix[matrix.index(start):
                            matrix.index(answer)]
            total = 0
            for e in range(len_e):
                total += equation[e]*solution[e]
                print(total)
            if total != answer:
                return False
    return True
...
#            base  exponent
def exponential(x, n, f=lambda x,n:x**n,rangen=None,interval=1):
    if rangen==None:
        rangen = range_x(n)
    else:
        rangen = rangen
    #rangen is whole numbers
    #this makes it variable to the interval
    rangen=rangen/interval
    if type(rangen)!=int:
        rangen=int(rangen)
...
##var_input(points=exponential(x=2/5,
##                             n=0,
...
##var_input(points=[[50,0],[0,10]],
##          color=GREEN)
##var_input(points=[[40,0],[0,5]],
##          color=BLUE)
##var_input(points=graph_function(-10,
##                                fx=lambda x:(25-x**2)**1/2))
##var_input(points= graph_function(-50,
##                                 fx=lambda x:-(x**4),
##                                 bools=[],
##                                 range_len=200,
...
##          color=CYAN)
##
##var_input(points=exponential(3,
##                             0,
##                             lambda x,n:math.log(n,x),
##                             rangen=-10),
##          color=PURPLE)

##var_input(points=graph_function(-1,
##                                lambda x:(math.e**x)+2))
                                
var_input(points=graph_function(0,
                                lambda x:3**x+4,
                                range_len=10),
          color=GREEN)
...
0.3.05
    ...
    #Different Functions and Formulas


0.7.03
    -Multivariable Linear Systems
    #given a list of variables, if points match all
      equations in the system, then return true
...
Do inputs for functions, save lambdas to temporary text.py
    ...
    -put in basic function text, but user inputs certain
...
while not done:
    ...
    for obj in POINTS:
        try:
            ...
            if not printed_overflow:
                print('Overflow Error, please use smaller values')
                printed_overflow=False
            #next iteration, skip this iteration
            pass
